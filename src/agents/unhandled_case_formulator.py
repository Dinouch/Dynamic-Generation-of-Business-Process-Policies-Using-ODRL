"""
unhandled_case_formulator.py — Agent 2 : Unhandled Case Formulator
...
@dataclass
class UnhandledCaseProposal:
    """
    One LLM-generated proposal for an unhandled BPMN pattern.

    Attributes
    ----------
    pattern_type
        Same identifier as ``UnhandledPattern.pattern_type``.
    ...
    @staticmethod
    def from_dict(d: dict) -> "UnhandledCaseProposal":
...
class UnhandledCaseFormulator:
    """
    Agent 2 — formulates ODRL hints for unhandled BPMN patterns.

    Receives ``GRAPH_READY`` from Agent 3 (when ``unhandled_patterns`` is non-empty).
    Emits ``UNHANDLED_PROPOSALS`` to Agent 3.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._last_proposals: list[UnhandledCaseProposal] = []
    ...
    def receive(self, msg: AgentMessage) -> None:
        ...
        if msg.msg_type == MessageType.GRAPH_READY:
            ...
            print(f"[Agent 2][WARN] Unhandled message type '{msg.msg_type.value}'.")

    def _handle_graph_ready(self, msg: AgentMessage) -> None:
        """Process ``GRAPH_READY`` and emit ``UNHANDLED_PROPOSALS``."""
        enriched_graph: EnrichedGraph = msg.payload["enriched_graph"]
        self._last_graph = enriched_graph
        self._reformulate_count = 0

        unhandled = list(getattr(enriched_graph, "unhandled_patterns", []) or [])
        proposals: list[UnhandledCaseProposal] = []

        if not unhandled:
            self._emit_unhandled_proposals(enriched_graph, proposals, msg.loop_turn)
            return
    ...
    def _emit_unhandled_proposals(
        self,
        enriched_graph: EnrichedGraph,
        proposals: list[UnhandledCaseProposal],
        loop_turn: int,
    ) -> None:
        """Send ``UNHANDLED_PROPOSALS`` to Agent 3."""
        self.send(
            AgentMessage(
                sender=self.AGENT_NAME,
                recipient="agent3",
                msg_type=MessageType.UNHANDLED_PROPOSALS,
                payload={
                    "enriched_graph": enriched_graph,
                    "unhandled_proposals": [p.to_dict() for p in proposals],
                },
                loop_turn=loop_turn,
            )
        )
    ...
    def _handle_reformulate(self, msg: AgentMessage) -> None:
        ...
        if self._reformulate_count >= self.MAX_REFORMULATE:
            ...
            if self._last_graph:
                self._emit_unhandled_proposals(
                    self._last_graph,
        ...
        for u in self._last_graph.unhandled_patterns:
    ...
    def _formulate_one(self, u, b2p_json: str) -> list[UnhandledCaseProposal]:
        """
        Call the LLM once for a single ``UnhandledPattern``.

        Parameters
        ----------
        u
            ``UnhandledPattern`` instance from Agent 1.
        b2p_json
            JSON string of B2P policies.

        Returns
        -------
        Zero or more ``UnhandledCaseProposal`` instances (one per policy in the
...
"""
        ...
        out: list[UnhandledCaseProposal] = []
    ...
    def _fallback_proposal(self, u: UnhandledPattern) -> Optional[UnhandledCaseProposal]:
        """
