...
Single responsibility:
    Validate ODRL JSON-LD policies generated by Agent 4
    at two verification levels, all deterministic, no LLM.

═══════════════════════════════════════════════════════════════════════
LEVEL 1 — ODRL SYNTAX (JSON-LD structure)
───────────────────────────────────────────
Validation via SHACL (pyshacl + pyld) when available,
fallback to manual checks otherwise.

Checks that each policy conforms to the W3C ODRL 2.2 schema:
  - Required fields: @context, uid, @type
  - Valid @type: Agreement | Offer | Set
  - At least one rule (permission | prohibition | obligation)
  - Each rule has: uid, target, action
  - All UIDs are well-formed URIs (http:// or https://)
  - No duplicate uid within the same policy
  - Each constraint has 3 fields: leftOperand, operator, rightOperand
  - Each refinement has 3 fields: leftOperand, operator, rightOperand

Syntax score: 1 - (N_errors / N_checks)   [AgentODRL formula]

Note on uid / @id:
  In the official ODRL JSON-LD context, "uid" is an alias for "@id".
  pyld expand() therefore does not produce a distinct odrl:uid triple.
  _dedupe_uid_atid_collision() removes redundant "uid" before pyld
  to avoid the "colliding keywords" error.
  odrl_shapes.ttl uses sh:nodeKind sh:IRI on NodeShapes
  (not sh:path odrl:uid) to align with this behavior.

═══════════════════════════════════════════════════════════════════════
LEVEL 4 — GLOBAL COHERENCE (intra + inter fragments)
────────────────────────────────────────────────────────
  - Permission/prohibition conflict on the same target
  - FPd XOR with two identical conditions
  - FPd XOR without conditional refinement
  - FPd AND/OR targeting unindexed rules
  - Activities referenced in FPd without local FPa
  - Message flows to nonexistent fragments
  - Assignee / duty target not found in global index
  - Inter-fragment dependency cycles

═══════════════════════════════════════════════════════════════════════
Levels 2 and 3 (FPa/FPd semantic): delegated to Agent 3.

Outputs:
    AuditIssue          : one individual anomaly
    AuditReport         : full report — is_valid if no CRITICAL

Multi-agent layer:
    - receive()  : accepts SYNTAX_AUDIT_REQUEST or POLICIES_READY from Agent 4 (re-audit)
    - send()     : emits ODRL_SYNTAX_FAILURE / ODRL_VALID / ODRL_SYNTAX_ERROR to Agent 4
    - Syntax loop (max MAX_SYNTAX_LOOPS turns):
...
class IssueLayer(Enum):
    SYNTAX = "syntax"   # Level 1 (SHACL or manual fallback)
    GLOBAL = "global"   # Level 4 — inter/intra-fragment coherence
...
class PolicyAuditor:
    ...
    def _audit_and_route(self, loop_turn: int) -> None:
        n_pol = sum(len(fps.all_policies()) for fps in (self.fp_results or {}).values())
        print(
            f"[Agent 5] Syntax / coherence audit — {n_pol} policy/policies, "
            f"round #{self._syntax_loop_count + 1}…"
        )
        report = self.audit()
    ...
    def _build_indexes(self) -> None:
        ...
        if self.validation_report:
            for p in getattr(self.validation_report, "accepted_unmapped_proposals", []) or []:
    ...
    def _check_rule_syntax(
        ...
        if not rule.get("action"):
            issues.append(issue(
                ...
                "action missing.",
                "Add an ODRL action (e.g. odrl:use, odrl:execute) per W3C ODRL Action vocabulary.",
                {"path": path},
            ))
        else:
